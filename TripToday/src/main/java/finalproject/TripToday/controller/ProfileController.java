...
import java.util.*; // Import Comparator și HashSet
...
public class ProfileController {
    ...
    private void addPageTextToModel(Model model) {
        // ... (metoda ta addPageTextToModel rămâne neschimbată) ...
    ...
    public String profile(Model model,
        ...
        String userEmail = principal.getEmail(); // Păstrăm email-ul dacă e nevoie pentru altceva
        ...
        try {
            ...
            List<Trip> enrolledUpcomingTrips = enrolledSplitTrips.getOrDefault("upcomingTrips", Collections.emptyList());
            List<Trip> enrolledPastTrips = enrolledSplitTrips.getOrDefault("pastTrips", Collections.emptyList());

            Set<Trip> finalUpcomingTripsSet = new HashSet<>(enrolledUpcomingTrips);
            Set<Trip> finalPastTripsSet = new HashSet<>(enrolledPastTrips);


            if (isGuide) {

                List<Trip> allSystemUpcomingTrips = tripService.getUpcomingTrips();
                for (Trip trip : allSystemUpcomingTrips) {

                    if (userId.equals(String.valueOf(trip.getGuideId()))) {
                        finalUpcomingTripsSet.add(trip);
                    }
                }

                List<Trip> allSystemPastTrips = tripService.getPastTrips();
                for (Trip trip : allSystemPastTrips) {
                    if (userId.equals(String.valueOf(trip.getGuideId()))) {
                        finalPastTripsSet.add(trip);
                    }
                }
            }


            List<Trip> finalUpcomingTrips = new ArrayList<>(finalUpcomingTripsSet);
            finalUpcomingTrips.sort(Comparator.comparing(Trip::getDepartureDate));

            List<Trip> finalPastTrips = new ArrayList<>(finalPastTripsSet);
            finalPastTrips.sort(Comparator.comparing(Trip::getDepartureDate).reversed());
            ...
            int totalUpcomingTrips = finalUpcomingTrips.size();
            ...
            List<Trip> pageUpcomingTrips = (upcomingFromIndex < totalUpcomingTrips && upcomingFromIndex >=0) ?
                    finalUpcomingTrips.subList(upcomingFromIndex, upcomingToIndex) : Collections.emptyList();
            List<String> upcomingTripGuideEmailsOnPage = pageUpcomingTrips.stream()
                    .map(trip -> trip.getGuideId() != null ? guideEmailMap.get(String.valueOf(trip.getGuideId())) : null)
            ...
            int totalPastTrips = finalPastTrips.size();
            ...
            List<Trip> pagePastTrips = (pastFromIndex < totalPastTrips && pastFromIndex >= 0) ?
                    finalPastTrips.subList(pastFromIndex, pastToIndex) : Collections.emptyList(); // Modificat
